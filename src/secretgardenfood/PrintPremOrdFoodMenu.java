...
/**
 * Print Premium Reservation = Order Food in Advance Class
 *
 * @author Japhet Mert Catilo Obsioma
 * @version 3.00, 10/28/2020
 */
public class PrintPremOrdFoodMenu {
    ...
    public static void print() throws IOException {
        ...
        try {
            ...
            PdfWriter writer = PdfWriter.getInstance(document,
                    new FileOutputStream("receipts/PremReservationFoodOrder/"
                            + java.time.LocalDate.now() + "-"
                            + PremCusReservation.getNewReceipt().getReceiptNo() + ".pdf"));
            document.open();
            ...
            PdfPCell cellCustomerInfo = new PdfPCell(new Paragraph(
                    "ID: " + PremCustomer.getNewCustomer().getCustomerID()
                    + "\n\nName: " + PremCustomer.getNewCustomer().getCustomerName()
                    + "\n\nPhone: " + PremCustomer.getNewCustomer().getCustomerPhone()
                    + "\n\nCard Number: " + PremCustomer.getPremiumCus().getPremiumCardNumber()));
            ...
            double newTotal = OrdFoodMenu.getNewFoodOrder().getFoodOrderTotal() + 30;
            ...
            PdfPCell cellReservationInfo = new PdfPCell(new Paragraph(
                    "Reservation ID: " + PremCustomer.getPremReservation().getReservationID()
                    + "\n\nReservation Info: " + PremCustomer.getPremReservation().getInfo()
                    + "\n\nNo of Persons: " + PremCustomer.getPremReservation().getNoPersons()
                    + "\n\nReservation Date: " + PremCusReservation.getNewRoomTable().getDate()
                    + "\n\nReservation Place: " + PremCusReservation.getNewRoomTable().getRoomName()
                    + "\n\nReservation Time: " + PremCusReservation.getNewRoomTable().getTime()
                    + "\n\nReservation Table: " + PremCusReservation.getNewRoomTable().getTableName()));
            ...
            tblItems.addCell(PremCusReservation.getNewReceipt().getReceiptType());
            tblItems.addCell("1");
            tblItems.addCell(String.valueOf(PremCustomer.getPremReservation().getCost()));
            ...
            document.close();
            writer.close();
            Desktop.getDesktop().open(new File("receipts/PremReservationFoodOrder/"
                    + java.time.LocalDate.now() + "-"
                    + PremCusReservation.getNewReceipt().getReceiptNo() + ".pdf"));
