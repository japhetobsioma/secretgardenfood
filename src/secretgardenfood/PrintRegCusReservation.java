...
/**
 * Print Regular Customer Reservation Class
 *
 * @author Japhet Mert Catilo Obsioma
 * @version 3.00, 10/28/2020
 */
public class PrintRegCusReservation {

    public static void print() throws IOException {
        Document document = new Document();
        try {
            PdfWriter writer = PdfWriter.getInstance(document,
                    new FileOutputStream("receipts/RegCusReservation/"
            ...
            tblItems.addCell(RegCusReservation.getNewReceipt().getReceiptType());
            tblItems.addCell("1");
            ...
            document.close();
            writer.close();
            Desktop.getDesktop().open(new File("receipts/RegCusReservation/"
                    + java.time.LocalDate.now() + "-"
                    + RegCusReservation.getNewReceipt().getReceiptNo() + ".pdf"));
