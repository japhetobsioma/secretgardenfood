...
/**
 * Print Renew Membership Class
 *
 * @author Japhet Mert Catilo Obsioma
 * @version 3.00, 10/28/2020
 */
public class PrintRenewMembership {
    ...
    public static void print() throws IOException {
        ...
        try {
            ...
            PdfWriter writer = PdfWriter.getInstance(document,
                    new FileOutputStream("receipts/RenewMembership/"
                            + java.time.LocalDate.now() + "-"
                            + RnwMembership.getNewReceipt().getReceiptNo() + ".pdf"));
            document.open();
            ...
            PdfPCell cellCustomerInfo = new PdfPCell(new Paragraph(
                    "ID: " + RnwMembership.getPremCusRenew().getCustomerID()
                    + "\n\nName: " + RnwMembership.getPremCusRenew().getCustomerName()
                    + "\n\nPhone: " + RnwMembership.getPremCusRenew().getCustomerPhone()
                    + "\n\nCard Number: " + RnwMembership.getPremCusRenew().getPremiumCardNumber()
                    + "\n\nExpiration Date: " + RnwMembership.getPremCusRenew().getPremiumExpireDate()));
            ...
            tblItems.addCell(RnwMembership.getNewApply().getType());
            tblItems.addCell("1");
            tblItems.addCell(String.valueOf(RnwMembership.getNewApply().getCost()));
            ...
            document.close();
            writer.close();
            Desktop.getDesktop().open(new File("receipts/RenewMembership/"
                    + java.time.LocalDate.now() + "-"
                    + RnwMembership.getNewReceipt().getReceiptNo() + ".pdf"));
